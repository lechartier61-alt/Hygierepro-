...
import { classifyScan } from './scan-classifier.js';
...
async function recognizeInternal(buffer,{mode='dlc'}={}){
  ...
  const shouldRetry=mode==='invoice' ? (Number(first.confidence||0)<62||(!invoice.totalTtc&&!invoice.invoiceNo)||Number(invoice.lines?.length||0)<2) : mode==='auto' ? (Number(first.confidence||0)<68||(!trace.expiryDate&&!trace.lot&&!trace.barcode&&Number(invoice.lines?.length||0)<2&&!invoice.invoiceNo)) : (Number(first.confidence||0)<62||!trace.expiryDate||!trace.lot||!trace.product);
  if(shouldRetry){
  ...
  const result={...trace,ocrConfidence,confidence:trace.confidence,invoice,ocrPasses:passes};
  return {...result,classification:classifyScan(result)};
}
