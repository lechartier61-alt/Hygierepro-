...
r.get('/dashboard',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  const [orgs,users,sessions,payments,storage,records,commerce,recent,versions,incidents,security]=await Promise.all([
    ...
    q(`SELECT count(*)::int total,
      ...
      count(*) FILTER (WHERE NOT email_verified)::int unverified,
      count(*) FILTER (WHERE role='owner')::int owners,
      count(*) FILTER (WHERE role='manager')::int managers,
      count(*) FILTER (WHERE role='employee')::int employees,
      count(*) FILTER (WHERE last_login_at>=now()-interval '24 hours')::int logged_24h,
      count(*) FILTER (WHERE created_at>=now()-interval '7 days')::int created_7d
      FROM users`),
    ...
    q(`SELECT o.id,o.name,o.city,o.created_at,o.last_activity_at,o.subscription_status,o.status,o.trial_ends_at,o.onboarding_completed,o.app_version,
      (SELECT count(*) FROM users u WHERE u.organization_id=o.id)::int users,
...
r.get('/organizations',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  const {rows}=await q(`SELECT o.*,
    ...
    (SELECT count(*) FROM users x WHERE x.organization_id=o.id)::int users_count,
    (SELECT count(*) FROM users x WHERE x.organization_id=o.id AND x.active)::int active_users_count,
    (SELECT count(*) FROM users x WHERE x.organization_id=o.id AND x.email_verified)::int verified_users_count,
    ...
    LEFT JOIN LATERAL (SELECT email,name,last_login_at FROM users WHERE organization_id=o.id AND role='owner' ORDER BY created_at LIMIT 1) u ON true
...
r.get('/organizations/:id/detail',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  ...
  const [users,recordTypes,recentRecords,payments,commerce,audits,schedules]=await Promise.all([
    q(`SELECT u.id,u.name,u.email,u.role,u.active,u.email_verified,u.last_login_at,u.created_at,
      ...
      ) s ON true WHERE u.organization_id=$1 ORDER BY u.role,u.name`,[id]),
...
r.get('/users',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  const {rows}=await q(`SELECT u.id,u.name,u.email,u.role,u.active,u.email_verified,u.last_login_at,u.created_at,
    o.id organization_id,o.name organization_name,o.status organization_status,o.subscription_status,
    COALESCE(s.active_sessions,0)::int active_sessions,s.last_seen_at,
    (SELECT count(*) FROM records r WHERE r.created_by=u.id)::int records_created
    FROM users u JOIN organizations o ON o.id=u.organization_id AND o.status<>'deleted'
    LEFT JOIN LATERAL (
      SELECT count(*) FILTER (WHERE expires_at>now()) active_sessions,max(last_seen_at) last_seen_at
      FROM sessions WHERE user_id=u.id
    ) s ON true
    ORDER BY COALESCE(s.last_seen_at,u.last_login_at,u.created_at) DESC NULLS LAST LIMIT 1000`);
  res.json(rows);
}));
...
r.delete('/organizations/:id',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  ...
  const org=(await q('SELECT stripe_subscription_id FROM organizations WHERE id=$1',[id])).rows[0];
  ...
  await audit(req,'admin.organization_delete','organization',id);await q('DELETE FROM organizations WHERE id=$1',[id]);res.json({ok:true});
}));
...
r.patch('/site-settings',requireAdmin,require2fa,asyncRoute(async(req,res)=>{
  ...
  if(d.legal){const email=String(d.legal.email||'').trim(),phone=String(d.legal.phone||'').trim();if(!z.string().email().safeParse(email).success||phone.length<6)throw new HttpError(400,'Renseignez un e-mail de contact valide et un numéro de téléphone joignable.','legal_contact_required');if(!d.legal.privacyEmail)d.legal.privacyEmail=email;}
