...
r.get('/settings',asyncRoute(async(req,res)=>{
  ...
  const legalMissing=['email','phone','publisher','privacyEmail'].filter(k=>!String(legal[k]||'').trim());
  res.json({brand:'HygieSafe',price:{amountCents:config.stripe.amountCents,currency:config.stripe.currency,trialDays:14,taxBehavior:'exclusive',label:'HT'},hero:{title:s.hero_title,subtitle:s.hero_subtitle,videoUrl:s.hero_video_url,fallbackUrl:s.hero_fallback_url},supportEmail:s.support_email||legal.email||'',legal,legalReady:legalMissing.length===0,legalMissing});
}));
export default r;
