...
r.get('/status',requireUser,asyncRoute(async(req,res)=>{
  const org=(await q(`SELECT trial_started_at,trial_ends_at,subscription_status,monthly_amount_cents,current_period_end,promo_code,stripe_customer_id FROM organizations WHERE id=$1`,[req.user.organization_id])).rows[0];
  res.json({...org,stripeConfigured:!!config.stripe.secretKey,trialDaysLeft:Math.max(0,Math.ceil((new Date(org.trial_ends_at)-Date.now())/86400000))});
}));
...
r.post('/checkout',requireUser,roles('owner'),asyncRoute(async(req,res)=>{
  ...
  const org=(await q('SELECT * FROM organizations WHERE id=$1',[req.user.organization_id])).rows[0];
  ...
  const metadata={organization_id:org.id,promo_code:promo?.code||'',promo_reservation_id:reservation?.id||'',amount_cents:String(amount)};
  ...
  try{
    session=await stripe.checkout.sessions.create({
      ...
      line_items:[{quantity:1,price_data:{currency:config.stripe.currency,unit_amount:amount,recurring:{interval:'month'},product_data:{name:'HygieSafe — 1 entreprise'}}}],
...
r.post('/portal',requireUser,roles('owner'),asyncRoute(async(req,res)=>{
  ...
  const org=(await q('SELECT stripe_customer_id FROM organizations WHERE id=$1',[req.user.organization_id])).rows[0];
