...
export const config = {
  ...
  stripe: {
    ...
    taxBehavior: process.env.STRIPE_PRICE_TAX_BEHAVIOR || 'exclusive',
...
export function validateProductionConfig(){
  ...
  if(config.stripe.secretKey&&config.stripe.webhookSecret&&!config.stripe.automaticTax) errors.push('STRIPE_AUTOMATIC_TAX doit être true lorsque Stripe est activé en production.');
  if(config.stripe.secretKey&&config.stripe.taxBehavior!=='exclusive') errors.push('STRIPE_PRICE_TAX_BEHAVIOR doit être exclusive pour le tarif B2B 9,99 € HT.');
