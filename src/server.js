...
import pilotageRoutes, { sensorIngestRouter } from './routes/pilotage.js';
...
app.use('/api',apiLimiter,sameOrigin,loadUser,loadAdmin,(req,res,next)=>{
  ...
  // Les points d'entrée publics d'authentification ne doivent jamais dépendre du
  // CSRF d'une session déjà présente dans le navigateur. Un utilisateur peut,
  // par exemple, ouvrir "Mot de passe oublié" alors qu'un ancien cookie de
  // session valide existe encore. Ces routes restent protégées par sameOrigin
  // et leurs limiteurs dédiés ; elles ne modifient pas la session courante sans
  // authentification explicite ou jeton à usage unique.
  const publicAuthWithoutCsrf =
    req.path==='/auth/login' ||
  ...
  if(publicAuthWithoutCsrf)return next();
...
app.use('/api/workdays',requireVerified,requireSubscription,workdayRoutes);
app.use('/api/equipment',requireVerified,requireSubscription,equipmentRoutes);
app.use('/api/pilotage/sensors/ingest',sensorIngestRouter);
app.use('/api/pilotage',requireVerified,requireSubscription,pilotageRoutes);
