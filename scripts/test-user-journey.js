...
test('Application mobile : navigation essentielle avec Ma journée',(app.match(/<nav class="mobile-nav">[\s\S]*?<\/nav>/)||[''])[0].match(/data-page=/g)?.length===6);
test('Scanner : DLC, facture, code-barres',app.includes('id="autoScanCamera"')&&app.includes('Scanner une DLC')&&app.includes('data-scan="invoice"')&&app.includes('data-scan="barcode"'));
...
test('Prise en main : accueil adapté au rôle',app.includes('id="dashboardRolePanel"')&&appJs.includes('quickByRole')&&appJs.includes('Pilotage établissement')&&appJs.includes('Supervision du service'));
