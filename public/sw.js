...
self.addEventListener('fetch',e=>{
  ...
  const codeAsset=/\.(?:css|js)$/i.test(u.pathname);
  if(codeAsset){
    e.respondWith(fetch(req).then(r=>{if(r.ok)caches.open(CACHE).then(c=>c.put(req,r.clone()));return r}).catch(()=>caches.match(req)));return;
  }
  const staticAsset=/\.(?:png|jpg|jpeg|webp|svg|woff2?)$/i.test(u.pathname);
...
self.addEventListener('notificationclick',event=>{event.notification.close();const page=event.notification?.data?.page||'actions';const target=`/app.html#${encodeURIComponent(page)}`;event.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(list=>{const existing=list.find(c=>new URL(c.url).origin===self.location.origin);if(existing){existing.navigate(target);return existing.focus()}return clients.openWindow(target)}))});
