...
async function saveLegal(e){e.preventDefault();const f=new FormData(e.currentTarget),legal={...(siteSettings?.legal||{})};for(const n of ['companyName','form','capital','rcs','rne','vat','publisher','address','email','phone'])legal[n]=String(f.get(n)||'').trim();if(!legal.email||!legal.phone)return alert('Renseignez un e-mail de contact et un téléphone avant d’enregistrer.');if(!legal.privacyEmail)legal.privacyEmail=legal.email;siteSettings=await api('/api/admin/site-settings',{method:'PATCH',body:{legal}});alert('Informations juridiques enregistrées')}
